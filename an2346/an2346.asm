...
;*                                                                             *
;*  The main subroutine "WriteBlock" is the same as in the Application         *
;*  Note code except that it calls "EEEPage" instead of the ROM                *
;*  subroutine "EraRnge". As long as this change is made to                    *
;*  WriteBlock, the only aditional code required is "EEEPage" and              *
;*  "EEEinRAM". The Flash reading routime "ReadBlock" and the                  *
...
ProgramRange        equ       $2809               ;Flash programming routine in ROM
...
ctrl_byte           rmb       1                   ;control byte for ROM subroutines
cpu_speed           rmb       1                   ;CPU speed in units of 0.25MHz
last_addr           rmb       2                   ;last Flash address to be programmed
...
;*******************************************************************************
;*  ReadBlock - Reads a block of data from Flash and puts it in RAM            *
;*                                                                             *
...
;*******************************************************************************
;*  WriteBlock - Writes a block of data into Flash from RAM buffer             *
;*                                                                             *
;*  Calling convention:    ldhx   #Blk1page                                    *
;*                         lda    #Blk1Size                                    *
;*                         jsr    WriteBlock                                   *
...
;*  Returns: nothing                                                           *
;*                                                                             *
;*  Uses:    FindClear, EEEPage, EEEinRAM (RAM), ProgramRange)                 *
;*******************************************************************************

                    #spauto

WriteBlock          proc
                    mov       #13,cpu_speed       ;3.2MHz/0.25MHz = 13
                    clr       ctrl_byte           ;page (not mass) erase
...
Found@@             pula                          ;get block size
                    ...
                    sthx      last_addr           ;save in RAM for use by ROM routine
                    pulx                          ;restore X (H hasn't changed)
                    jmp       ProgramRange        ;program block (includes RTS)
